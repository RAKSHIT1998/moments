...
**Decentralised by design — no MOMENT servers, no sign-up, no data collection.** There is no backend we run. Identity is the user's iCloud account; every Moment lives in its creator's own iCloud private database and is shared peer-to-peer through CloudKit shares (`CKShare`) that only the invited people can open. Public Moments are the one thing everyone can see, by the creator's explicit choice, and they carry only a venue — never a person's location. Analytics are local counters that never leave the phone, off by default. `.moment` packages let two phones exchange a Moment with no network at all (AirDrop/Messages). The `SocialBackend` protocol is the seam for any other transport later (Nostr-style relays, iCloud-free P2P).
