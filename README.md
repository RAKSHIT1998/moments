...
Creators have **one subscription at a price they choose**, sell **sets** (photos/clips, free or priced), and answer **asks** — a fan requests a photo, a call, a meeting or anything else, and the creator names a price for that one thing (`asked → quoted → requested → accepted`; money counts only from `accepted`). Also: **bundles** (3/6/12 months at the creator's discount), **locked messages** (a priced photo in a chat — a hidden one-item set, so the media never travels unpaid), **mass messages** to every active subscriber in their own chat, and **tip goals** whose progress is the real total tipped. Studio shows where the month's money came from and the top supporters. Paid media is capture-resistant on iOS (screenshots and recordings come out blank) and watermarked with the viewer's MOMENT ID; the creator is told when someone captures. Nothing stops a second camera — the app says so instead of pretending. Studio (Profile → *Sell your photos and your time*) shows this month's earnings **with the rail named**, because the take rate differs: on card checkout MOMENT keeps 10%; through Apple IAP, Apple takes 30% first and the creator keeps 80% of the rest. A set's window (title, price, cover) is public; its contents are sealed under a per‑set key that the creator's phone hands to buyers only — a third device with every byte can open nothing (`DecentralizedStorefrontTests`). Withdrawing rotates the key. Full economics, the adult‑content constraints and what's still missing before money moves: `CREATOR-PLATFORM.md`.
